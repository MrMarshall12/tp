...
6. **Support error specification for `schedule` and `reschedule` commands without dates of each day of week specified in the date range**: If the date range of the delivery does not contain each of the days of the week for the delivery, ServeMate will specify that this issue has occurred. This allows you to immediately notified of the issue and rectify it.
